...
- 甲方简版分支流程（按设置项）：
  - 入口：点击悬浮按钮 `运行`
  - 分支A（AI）：
    - 若 `AI大模型分析消费数据=关闭`：跳过 AI 连接测试与 AI 分析。
    - 若 `AI大模型分析消费数据=开启`：先做 AI 测试连接；失败则阻断本次运行并提示失败详情。
  - 分支B（飞书）：
    - 若 `飞书机器人发送数据结果=关闭`：跳过飞书连接测试与飞书发送。
    - 若 `飞书机器人发送数据结果=开启`：做飞书连接测试；失败则阻断本次运行并提示失败详情。
  - 分支C（自动运行）：
    - `启动软件自动运行模块=开启`：打开 LOOK 后自动进入 RUNNING。
    - `启动软件自动运行模块=关闭`：需手动点击“运行”。
  - 分支D（榜单采集模式）：
    - `全量采集榜单用户数据=开启`：忽略贡献/魅力榜循环点击次数，持续上滑采集。
    - `全量采集榜单用户数据=关闭`：按“贡献榜列表组循环点击次数/魅力榜列表组循环点击次数”采集。
    - 当全量模式开启时：
      - `全量采集Data数值限制=0`：不做 Data 阈值截断。
      - `全量采集Data数值限制>0`：先做“万/亿”换算，再比较；小于阈值即结束当前榜单采集。
  - 分支E（用户详情）：
    - `采集用户详细界面=开启`：进入用户详情补充ID/IP等字段。
    - `采集用户详细界面=关闭`：仅采榜单列表可见字段。
  - 分支F（广告处理）：
    - `广告处理=开启`：执行规则引擎。
    - `广告处理=关闭`：跳过广告规则处理。
    - `无障碍实时广告服务（替代LSP）=开启`：广告处理由无障碍实时服务执行。
  - 分支G（调试）：
    - `View树调试输出（仅调试）=开启`：输出更多界面树调试日志与导树能力。
  - 采集完成后：
    - 贡献榜 + 魅力榜均成功时，若 AI 开启则发起 AI 分析。
    - AI 有有效记录：写入 AI 结果 CSV；若飞书开关开启则按用户逐条推送飞书。
    - AI 无有效记录：不写 AI 结果 CSV，不发送飞书。
- 悬浮按钮状态文案（v1.0.33）：
  - 运行中：`LSP-运行中`
  - 未运行：`LSP-未运行`
- `运行`命令补发机制（v1.0.17）：
...
- AI 消费分析与飞书推送（v2.2.5+ 持续增强）：
  ...
  - AI 结果文件（v2.2.28 起）：
    - 统一保存到 `look_rank_csv/` 目录，与榜单 CSV 同目录。
    - 文件名：`look_ai_consumption_analysis_yyyyMMdd.csv`（按天聚合）。
    - 标题列：`用户昵称,用户ID,用户等级,消费数据,消费计算准确度,消费对象昵称,消费对象ID,直播间ID,在直播间总消费数据,写入时间`。
    - 仅写入“有效解析记录”；若仅有 `NO_VALID_RECORDS` 或空模板块，则不落盘。
