...
- 运行前置环境强校验与单榜重试上限（v2.1.9）：
  ...
  - 设置页新增 `单榜重试上限/超时重试次数`（默认 `3`），用于限制单榜采集失败后的重派发次数，避免在榜单数量不足时无限重试。
  - 当达到单榜重试上限后，`live_room_enter_task` 会停止当前榜单重派发并结束本次任务链路，日志标记 `rank_collect_retry_limit_reached`。
- 详情采集严格校验与日志增强（v2.2.0）：
  - 当“采集用户详细界面”开启时，用户详情采集结果新增日志：`enteredDetail/valid/id/ip/consumption/followers`，便于排查详情页识别与取值。
  - 详情采集无效（例如进入详情后仍未拿到有效字段）不再继续按成功行写入 CSV，改为标记本轮榜单采集失败（`rank_detail_invalid`），走任务侧重派发重试链路。
  - 避免“详情模式开启但 CSV 仍写入空详情字段”的隐性数据污染。
